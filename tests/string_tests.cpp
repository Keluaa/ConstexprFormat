...
TEST_CASE("%s dynamic char array")
{
    ...
    {
        static constexpr auto fmt_str = "%s world"sv;
        constexpr auto fmt = cst_fmt::compile_format_string<fmt_str, cst_fmt::dyn_str<6>>();
        for (const char* str : { "hello", "bye" }) {
            auto result = cst_fmt::format(fmt, cst_fmt::dyn_str<6>{str});
    ...
    {
        static constexpr auto fmt_str = "%s world"sv;
        constexpr auto fmt = cst_fmt::compile_format_string<fmt_str, cst_fmt::dyn_str<0>>();

        const char* str = "";
        auto result = cst_fmt::format(fmt, cst_fmt::dyn_str<0>{str});
        std::string expected = std::string(str) + " world";
        CHECK_EQ(result, expected);
    }
}
